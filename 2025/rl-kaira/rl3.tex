...
特にロボット分野では、学習したモデルで外部環境を認識し、凹凸路や滑り面での歩行・バランス回復を組み立てるといった事例が増えています。
そして、それを応用して災害時の救助用ロボットが開発されたりしております。\\
今回は、歩くKaiRA君：強化学習を用いた二足歩行ロボットの歩行制御について解説します。
% --- 第2章 ---
\section{学習目標}
\label{sec:target}
本プロジェクトでは、Unity ML-Agentsを用いて、二足歩行ロボット（KaiRAくん）をシミュレーション環境で学習させる。\\
学習環境には目標オブジェクトが存在し、図\ref{fig:kaira-overview}の矢印の方向に置かれている。\\
...
図\ref{fig:quaternion}で表すと、ベクトル$r^{'}$はベクトル$r$を軸$\vec{n}$の周りに$\theta$だけ回転させたものであり、これを表すには軸の情報(3次元)と回転量(1次元)が必要です。\\
...
図\ref{fig:cumulative-reward}は学習の進行に伴う累積報酬の推移を示しています。学習初期には報酬が低く、ランダムな動作が多かったですが、学習が進むにつれて報酬が増加し、安定した歩行が実現されました。\\
\begin{figure}[h]
\begin{center}
